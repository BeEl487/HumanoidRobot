...
| Mount height above ground | **0.92 m (raised from 0.90 m, 2026-08-19)** | User-reported the arm's upper-arm/forearm segments grazing the box walls while reaching down -- at 0.90 m the shoulder sat only ~10.5 cm above the source/destination boxes' opening. Tried +0.10 m (1.00 m) first: dropped the bin-floor reachability gate from 100%->66.7% (arm too far above the workspace to reach the floor at a good angle) -- too aggressive. +0.04 m (0.94 m) still left the destination box's extreme far corner (Y=-0.365, already a marginal ~1.4-2cm-residual point even at 0.90 m) failing worse. Settled on **+0.02 m (0.92 m)**: `reachability_check.py`/`check_reach_collision.py` gates both pass with margin (88.9%, up from needing to verify at all), full 25-point IK grids over both boxes' actual spawn/placement footprints pass (destination box's extreme corner residual only marginally worse than its already-marginal 0.90 m baseline, not a new break), and 100% camera-frustum coverage of the source box's spawn range re-verified at the new camera height. `MOUNT_HEIGHT_M` in `scripts/reachability_check.py` kept in sync (duplicated constant, see that file's comment). | No — sim-only rig parameter, not a real robot spec (the real robot has no defined mounting/base height either). |
